...
search_btn.addEventListener('click', throttle((e) => {
    // 需要执行的代码
    getView(filter_name,filter_id,search)
}, 1000), false);

function getView(filter_name,filter_id,search) {
    viewBox.innerHTML = ''
    $.ajax({
        type: 'post',
        url: `/filter.php`,
...
$("span").click(function () {
    let filter_name = $(this)[0].attributes[0].nodeName
    let filter_id = $(this)[0].attributes[0].value
    getView(filter_name,filter_id,search)
})
